...
> TIP: See more details about these models in the package's [dbt docs site](https://fivetran.github.io/dbt_microsoft_ads/#!/overview?g_v=1&g_e=seeds).
...
The default behavior for the `microsoft_ads__url_report` end model is to filter out records having null URL fields, however you are able to turn off this filter if necessary. This is done by setting one of the variables below to `True` in your `dbt_project.yml` file. 
