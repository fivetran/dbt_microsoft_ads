...
- Added the `microsoft_ads__campaign_country_report` and `microsoft_ads__campaign_region_report` end models and upstream staging models. See above for schema change details and new models added.
  - For dbt Core users: If you do not sync this table or would like disable these new models you can disable the models by setting the  `microsoft_ads__using_geographic_daily_report` variable to `false` in your `dbt_project.yml` file (`true` by default). Refer to the [README](https://github.com/fivetran/dbt_microsoft_ads?tab=readme-ov-file#enable-geographic-reports) for more details. 
